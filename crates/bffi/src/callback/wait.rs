//! The marshal-and-wait dispatch of `invoke_wait`: the cross-thread
//! counterpart of a plain `invoke` that ANY native thread may call.
//!
//! Split out of `registry.rs` (which owns the two callback tables and
//! their register/bind/invoke/revoke surface): this module owns the
//! WAIT machinery - the shared hand-off slot, the C-call matrix of the
//! JS-bound dispatch, and the marshal jobs that run on the JS thread.
//! The whole module is feature-gated on `event-loop`: the wait rides
//! the event loop's queue.
//!
//! Split contract: `registry.rs` stays the single removal/lifecycle
//! point (`revoke`), and this module never mutates the tables - it
//! looks slots up fresh inside every job, so a revocation during the
//! wait surfaces as `InvalidHandle` through the slot.
...
use bffi_core::{Handle, Registry};

use super::error::CallbackError;
use super::registry::{JsEntry, invoke, tables};
use super::thread::{end_wait, ensure_js_thread, try_begin_wait};
...
#[cfg(test)]
mod tests {
    // NOTE (test isolation): these tests bind JS-bound handles and
    // touch the event-loop queue; the crate-wide lock keeps a parallel
    // test thread from flipping the global binding gate mid-test.
    use std::ffi::CStr;
    use std::os::raw::c_char;
    use std::sync::Arc;

    use super::invoke_wait;
    use crate::bffi_callback::CallbackError;
    use crate::bffi_callback::process_state_lock;
    use crate::bffi_callback::registry::{bind_js_callback, register};
