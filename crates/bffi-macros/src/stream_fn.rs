...
pub(crate) enum StreamItem {
    ...
    /// `i64` items.
    Int64,
    /// `u64` items - one exact `u64` wire record each (TAG 10).
    UInt64,
    /// `bool` items.
    Bool,
    /// `String` items.
...
impl StreamItem {
    ...
    fn encode_stmt(&self, wire: &TokenStream, ident: &syn::Ident) -> TokenStream {
        match self {
            ...
            Self::UInt64 => quote! { #wire::encode_u64(&mut rec, #ident); },
...
/// The parsed and validated `#[bffi_stream]` function: one of the
/// two shapes.
pub(crate) enum StreamModel {
    /// A pull stream: plain fn returning
    /// `impl Iterator<Item = T> + Send`.
    Pull(StreamFnModel),
    /// A push stream: async fn taking `Ctx<T>` first, producing
    /// items with backpressure on the async executor.
    Push(PushModel),
}

/// A validated push producer.
pub(crate) struct PushModel {
    /// Function identifier.
    pub ident: syn::Ident,
    /// Doc lines, one leading space trimmed.
    pub docs: Vec<String>,
    /// Path context.
    pub paths: PathCtx,
    /// The classified item kind (from the `Ctx<T>` annotation).
    pub item: StreamItem,
    /// The JS-visible parameters (everything after the `Ctx<T>`).
    pub params: Vec<(String, ShimKind)>,
}

/// Parses and validates the annotated stream function: shape
/// dispatch - a plain fn returning `impl Iterator` is a pull
/// stream; an async fn taking `Ctx<T>` first is a push producer.
pub(crate) fn parse(attrs: &TokenStream, item: TokenStream) -> syn::Result<StreamModel> {
    ...
    if func.sig.asyncness.is_some() {
        return parse_push(&func, paths);
    }
    parse_pull(&func, paths).map(StreamModel::Pull)
}

/// The push shape: `async fn(ctx: Ctx<T>, params...) -> ...`.
fn parse_push(func: &syn::ItemFn, paths: PathCtx) -> syn::Result<StreamModel> {
    let mut inputs = func.sig.inputs.iter();
    let Some(first) = inputs.next() else {
        return Err(stream_shape(
            func.sig.inputs.span(),
            "an async producer must take `Ctx<T>` as its first parameter",
        ));
    };
    let syn::FnArg::Typed(pat) = first else {
        return Err(stream_shape(
            first.span(),
            "the first parameter must be `Ctx<T>`",
        ));
    };
    let Some(item_ty) = ctx_item_type(&pat.ty) else {
        return Err(stream_shape(
            pat.ty.span(),
            "the first parameter must be `Ctx<T>` - the item type drives the descriptor",
        ));
    };
    let item = StreamItem::classify(item_ty)?;
...
/// The `T` of a `Ctx<T>` parameter type, when the type is a path
/// whose last segment is named `Ctx` with exactly one generic
/// argument.
fn ctx_item_type(ty: &syn::Type) -> Option<&syn::Type> {
    let syn::Type::Path(syn::TypePath { qself: None, path }) = ty else {
        return None;
    };
...
/// The pull shape: a plain fn returning `impl Iterator<Item = T>`.
fn parse_pull(func: &syn::ItemFn, paths: PathCtx) -> syn::Result<StreamFnModel> {
    let mut params = Vec::new();
...
/// The push descriptor module: same shape as the pull descriptor
/// (the JS-visible contract is identical).
pub(crate) fn push_meta(model: &PushModel) -> TokenStream {
    let module = format_ident!("bffi_meta_{}", model.ident);
    let js_name = model.ident.to_string();
    let export_name = format!("bffi_{}", model.ident);
    let module_doc = format!(
...
/// The push-shim expansion: creates the buffer + producer context,
/// spawns the producer future on the async executor, and writes the
/// stream handle to the out-parameter. The producer's `Err` return
/// fails the stream (the JS side sees the message).
pub(crate) fn expand_push(model: &PushModel) -> TokenStream {
    let shim_ident = format_ident!("bffi_{}", model.ident);
    let shim_doc = format!(
        "Spawns the push-stream producer `{}` and writes the stream handle to `__ret` (generated by `#[bffi_stream]`).",
        model.ident
    );
    let paths = &model.paths;
    let core = &paths.core;
    ...
    let ident = &model.ident;
    let spawn = quote! {
        let (__stream, __ctx) = match ::bffi::bffi_stream::spawn_push() {
        ...
        let __fut = async move {
            let __outcome = #ident(__ctx, #(#call_args,)*).await;
            match __outcome {
                ::std::result::Result::Ok(()) => {
                    let _ = __ctx.complete();
                }
                ::std::result::Result::Err(__err) => {
                    let _ = __ctx.fail(&::std::string::ToString::to_string(&__err));
                }
            }
        };
        match ::bffi::spawn(::std::boxed::Box::pin(async move {
            __fut.await;
            ::std::result::Result::Ok(#async_root::AsyncValue::Unit)
        })) {
            ::std::result::Result::Ok(__task) => {
                // The task handle is intentionally dropped: the
                // stream handle is the JS-visible identity, and the
                // producer ends on its own (or when the stream is
                // dropped - pushes then resolve Closed).
                let _ = __task;
                // SAFETY: `__ret` is non-null (checked above) and valid
                // for one `u64` write per the bun:ffi out-parameter
                // contract.
