...
| Composites (B1+B4) | Records/enums/`Vec<T>` (incl. `Vec<Vec<u8>>`) sync + async; data-carrying enum variants ride the kind envelope (`TAG_RECORD` of variant name + positional payload; TS `{ kind, ... }` discriminated union, discriminant `kind`, tuple fields `_0`..), unit-only enums keep the `TAG_STR` string union; `Option<Record>`/`Option<Vec<T>>` returns = `| null` over the 0-handle empty-buffer convention; item/field/variant matrix rejects deeper nesting |
