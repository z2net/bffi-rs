...
side then drives the stream through two generic exports expanded by
...
error: the JS wrapper waits a tick and retries. The buffer is
shared memory, so the pull itself is the delivery; no event-loop
