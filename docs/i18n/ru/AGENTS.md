...
- Релиз блокируется, пока не готова сопроводительная часть: запись в `CHANGELOG.md` для версии и обновлённая документация (реестровые README пакетов и крейтов, плюс `docs/DESIGN.md` / `crates/bffi/CALLING-CONVENTION.md`, когда менялось поведение или публичная поверхность). npm и crates.io замораживают эти страницы в момент публикации - правка документации после публикации требует новой версии. Если changelog или документация не написаны, релиз не готов.
...
9. Перед релизом (или просьбой о нём) проверить, что в `CHANGELOG.md` есть запись для версии и что документация (README пакетов и крейтов, DESIGN.md, CALLING-CONVENTION.md) отражает все внесённые изменения - если нет, сначала написать их; реестры замораживают эти страницы в момент публикации.
